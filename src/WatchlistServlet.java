...
@WebServlet(name = "WatchlistServlet", urlPatterns = "/api/watchlist")
public class WatchlistServlet extends HttpServlet {

    @Serial
    private static final long serialVersionUID = 1L;

    private static final Set<String> VALID_STATUSES = Set.of("watched", "watching", "plan_to_watch");
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            for (Movie movie : userDao.getWatchlist(username)) {
                watchlistArray.add(toJson(movie));
            }
            ...
            out.write(watchlistArray.toString());
            out.close();
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if (movie == null) {
                ...
                responseJson.addProperty("status", "error");
                responseJson.addProperty("message", "status must be one of " + VALID_STATUSES);
                response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            } else {
                UserDao userDao = DaoFactory.getUserDao();
                userDao.setWatchStatus(username, movie, status);
    ...
    protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if (movieId == null) {
                ...
                UserDao userDao = DaoFactory.getUserDao();
                userDao.removeFromWatchlist(username, movieId);
                responseJson.addProperty("status", "success");
                response.setStatus(HttpServletResponse.SC_OK);
            }
        } catch (Exception e) {
    ...
    private JsonObject toJson(Movie movie) {
        ...
        movieJson.addProperty("status", movie.getWatchStatus());
